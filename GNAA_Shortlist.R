...
quarterdate<-"2017-09-30"  # MAY NEED TO VERIFY DATE FORMAT...
...
aptdf<- read.csv("D:/Projects/GNAA/Data/3rdQtr2017Data/2017_09_30_AptsClean.csv", stringsAsFactors = FALSE)
# List of desired apartment ID's
unitmixall<- read_excel("D:/Projects/GNAA/Data/3rdQtr2017Data/export/UnitMixRents.xlsx")
colnames(unitmixall)<- gsub(" ", ".", colnames(unitmixall))
occupancymanagement<- read_excel("D:/Projects/GNAA/Data/3rdQtr2017Data/export/OccupancyManagement.xlsx")
...
getPropInf0<- function(inputID, AptPropList){
  ...
  prophtml<- htmlTable(propdf, rnames = FALSE, align = "left", align.header = "l")
...
NAME<- unlist(lapply(aptids, function(x) aptdf$NAME[which(aptdf$ID==x)]))
TAB_NAME <- unlist(lapply(aptids, function(x) aptdf$SUBMARKET[which(aptdf$ID==x)]))
...
THUMB_URL<-lapply(aptids, function(x) if(x%in%imgs01$aptID){
  ...
  "https://gnaa.maps.arcgis.com/sharing/rest/content/items/11a97fda98b94c21811c3540b0307e60/data"
  
})

PIC_URL<-lapply(aptids, function(x) if(x%in%imgs02$aptID){
  paste(rooturl, imgs02[which(imgs02$aptID==x),]$ImgName[1],sep = "")
}else{
  "https://gnaa.maps.arcgis.com/sharing/rest/content/items/11a97fda98b94c21811c3540b0307e60/data"
})
LAT<- lapply(aptids, function(x) aptdf$POINT_Y[which(aptdf$ID==x)])
LONG<- lapply(aptids, function(x) aptdf$POINT_X[which(aptdf$ID==x)])

#Combine all
tdf<- data.frame("ID"=unlist(aptids),
                 "NAME"=unlist(NAME),
